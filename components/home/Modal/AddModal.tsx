...
const AddModal = ({ todos, setTodos, isOpen, onClose }: IAddModal) => {
	const date = new Date()
	...
	const [data, setData] = useState({
		...
		judul_task: '',
		komentar_task: '',
		jam_task: `${tempHours}:${tempMinutes}`,
		tanggal_task: date.toISOString().substring(0, 10),
		status_task: 'todo', // 'todo' || 'done'
	...
	const onSubmit = () => {
		const dueDate = new Date(data.tanggal_task + ' ' + data.jam_task)
		...
		if (data) setTodos([newTodo, ...todos])
